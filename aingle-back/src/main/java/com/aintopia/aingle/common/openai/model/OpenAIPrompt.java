...
public enum OpenAIPrompt {
    ...
    AI_CHARACTER_CREATE_KEYWORD_PROMPT("이 사진은 사용자가 SNS에 올린 사진이야 사진(이미지)에 대한 상황 분석(5문장), 키워드(5개), 추가적인(추측성) 해석(3문장)을 다음 양식에 맞춰 정리해주세요\n"
        + "### 사용자가 SNS에 올린 게시글 ###\n"
        + "1. 각 항목에 해당하는 내용..."),
    AI_CHARACTER_CREATE_ANS_PROMPT("당신은 이 질문에 적절한 답변을 무조건 답해야 합니다. 다음에 제공되는 분석을 바탕으로 사용자의 SNS 게시글에 댓글을 작성해주세요. 분석 내용을 참고하여 당신의 성격, 성향, 특성을 반영한 댓글을 달아주세요. 당신은 캐릭터의 고유한 말투와 스타일에 맞아야 합니다.\n"
        + "\n"
        + "무엇을 하지 말아야 할지보다는, 무엇을 해야 하는지에 초점을 맞추세요. 사용자의 게시글에 맞춰, 나이에 적합한 언어, 인터넷 용어, 그리고 당신의 성격에 맞는 표현을 활용해주세요. 목표는 진정성 있고, 공감 가며, 맞춤형 댓글을 작성하는 것입니다."
        + "# 게시글\n"
        + "게시글과 함께 작성된 내용: %s\n"
        + "%s\n"
