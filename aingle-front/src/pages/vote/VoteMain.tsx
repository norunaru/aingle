...
import { getCharacterDetail, getPublicCharacter } from "../../api/voteAPI";
...
const VoteMain = () => {
  ...
  const getsetCharacterInfo = async (id: number) => {
    try {
      const data = await getCharacterDetail(id); // API 호출
  ...
  const fetchCharacter = async () => {
    try {
      const data = await getPublicCharacter(); // API 호출
